...
`warp-oss.exe` 依赖同目录下的 ConPTY 伪终端运行库，**必须放在一起**，否则启动报 `Failed to load ConPTY library module`：
...
`app/build.rs` 在构建时会自动把这些 DLL 拷到 `target/<profile>/` 下；分发时连同 exe 一起打包即可。便携版解压到任意目录（无需装在 C 盘）双击即用，卸载只需删除整个文件夹。

### 从源码构建（免 Visual Studio）

支持用便携 LLVM + xwin 工具链构建，不需要完整 Visual Studio、不写注册表、不碰系统策略：

```sh
cargo build --release --bin warp-oss --features gui
```

便携工具链需要 `clang-cl` / `lld-link` / `llvm-rc`（资源编译）/ `protoc`，并通过 `WARP_RC` 环境变量指定独立资源编译器（绕开对已注册 Windows SDK 的依赖）。详细环境配置见 `docs/building-windows-portable.md`（随 Windows 资源编译 PR 提供）。
...
Windows 与 Linux 同款：登录 token / AI API key / MCP 凭据以 AES-256-GCM 加密文件落盘（不使用任何系统钥匙串），见 `crates/warpui_extras/src/secure_storage/`。
